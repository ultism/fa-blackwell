// S0b: QK-only with warp specialization + cutlass PipelineTmaAsync ring.
// Builds on S0a (TMA+SF+atom proven bit-exact) by adding the real WS pipeline:
//   - 3 warpgroups: WG0 = producer (warp 0 issues TMA), WG1+WG2 = consumers
//     (256 threads = the 8-warp QK TiledMMA),
//   - Q loaded once via PipelineTmaAsync<1>, K+SFK streamed through a
//     PipelineTmaAsync<kStages> ring across multiple n_blocks,
//   - producer/consumer mbarrier handshake (acquire / get_barrier / wait /
//     release) -- the "can the pipeline schedule + not deadlock" check.
// seq_k = 2*kBlockN exercises the ring (kStages=2). Output S[kBlockM, seq_k]
// per n_block; validate vs fp64 dequant reference. No tile scheduler yet (S0c).
//
// Build:
//   nvcc -std=c++17 -O2 -gencode arch=compute_120a,code=sm_120a \
//     --expt-relaxed-constexpr --expt-extended-lambda \
//     -I tmp/cutlass/include -I include tests/s0b_qk_ws.cu -o tests/s0b_qk_ws
...
struct Params {
  ...
  int seqlen_k; int n_block_max;
  float* out_S;  // [kBlockM, seqlen_k]
};
...
__global__ void __launch_bounds__(kNThreads, 1) s0b_kernel(CUTE_GRID_CONSTANT Params const params) {
  ...
  if (warp_group_idx == 0) {
    ...
    if (warp_idx_in_wg == 0 && lane_predicate) {
      ...
      Tensor tKgK = group_modes<0, 3>(bk.partition_S(gK)); Tensor tKsK = group_modes<0, 3>(bk.partition_D(sK));       // (TMA, nb) / (TMA, stage)
    ...
    Tensor mS = make_tensor(make_gmem_ptr(params.out_S),
                            make_layout(make_shape(int(kBlockM), int(params.seqlen_k)), make_stride(int(params.seqlen_k), _1{})));
    ...
    for (int nb = 0; nb < n_block_max; ++nb) {
      ...
      Tensor gS_nb = local_tile(mS, select<0, 1>(TileShape_MNK{}), make_coord(_0{}, nb));
      Tensor tSgS = thr_mma.partition_C(gS_nb);
      Tensor accum = thr_mma.partition_fragment_C(gS_nb);
      clear(accum);
...
int main() {
  constexpr int n_block_max = 2;
  const int M = kBlockM, HD = kHeadDim, SK = n_block_max * kBlockN;
  printf("S0b: WS + PipelineTmaAsync ring, QK-only, seq_k=%d (%d n_blocks, kStages=%d)\n", SK, n_block_max, kStages);

  std::vector<float> dataVals = {0.5f, 1.f, 2.f, -0.5f, -1.f, -2.f};
  std::vector<uint8_t> hQ(M * HD), hK(SK * HD);
  std::vector<int> qexp(M * NBLK), kexp(SK * NBLK);
  for (int m = 0; m < M; ++m) for (int b = 0; b < NBLK; ++b) qexp[m * NBLK + b] = ((m + b) % 4) - 1;
  for (int n = 0; n < SK; ++n) for (int b = 0; b < NBLK; ++b) kexp[n * NBLK + b] = (n + 2 * b) % 3;
  for (int m = 0; m < M; ++m) for (int k = 0; k < HD; ++k) hQ[m * HD + k] = e4m3_byte(dataVals[(m * HD + k) % dataVals.size()]);
  for (int n = 0; n < SK; ++n) for (int k = 0; k < HD; ++k) hK[n * HD + k] = e4m3_byte(dataVals[(n * 7 + k * 3) % dataVals.size()]);

  auto layoutSFQ = BlkSF::tile_atom_to_shape_SFA(make_shape(M, kBlockN, HD));
  ...
  CK(cudaMalloc(&dS, M * SK * sizeof(float)));
  ...
  params.seqlen_k = SK; params.n_block_max = n_block_max; params.out_S = dS;

  int smem_bytes = int(sizeof(SharedStorage));
  CK(cudaFuncSetAttribute(s0b_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_bytes));
  printf("smem = %d bytes, threads = %d\n", smem_bytes, kNThreads);
  s0b_kernel<<<1, kNThreads, smem_bytes>>>(params);
  CK(cudaGetLastError());
  CK(cudaDeviceSynchronize());

  std::vector<float> hS(M * SK);
  CK(cudaMemcpy(hS.data(), dS, M * SK * sizeof(float), cudaMemcpyDeviceToHost));
